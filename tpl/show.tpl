	...
	$.ajax({
        ...
        success:function(data){
			...
			$.each(data,function(index,json){
				html += '<tr class="my_hover"><td style="width:120px;">'+json.site_name+'</td><td style="width:140px;">'+json.ad_name+'</td><td style="width:100px;">'+json.start_time+'</td><td class="end_time" style="width:100px;">'+json.end_time+'</td><td style="width:80px;"><a href="'+json.url+'" target="_blank"><span class="glyphicon glyphicon-flash"></span></a></td><td onclick="pic_show()" style="width:80px;"><span class="glyphicon glyphicon-picture"></span></td><td style="width:280px;">'+json.desc_info+'</td><td style="width:100px;">'+json.others+'</td><td style="width:106px;"><a class="btn btn-xs btn-primary" onclick="change_box(\''+json.id+'\')" data-toggle="modal" data-target=".change_ad">修 改</a> <a class="btn btn-xs btn-danger pull-right" onclick="remove_ad(\''+json.id+'\')">删 除</a></td></tr>';
			})
